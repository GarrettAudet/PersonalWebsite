...
function Hero() {
  return (
    <section className="hero" id="home" aria-labelledby="hero-title"><div className="hero-inner page-grid">
...
function About() {
  ...
  return <section className="section about" id="about" aria-labelledby="about-title"><div ref={revealRef} className={`section-inner about-grid${visible ? " visible" : ""}`}><div className="about-intro"><SectionLabel number="01">About Me</SectionLabel><h2 id="about-title">From public institutions to technical systems, I focus on turning complexity into clarity.</h2><a className="text-link" href="#experience">View Full Bio <FontAwesomeIcon icon={faArrowRight} /></a></div><ul className="capability-list">{capabilities.map((item, index) => <li key={item} style={{ "--item": index }}><span aria-hidden="true">+</span>{item}</li>)}</ul><ImpactStack items={stackItems} /></div></section>;
}
...
function Projects() {
  ...
  return <section className="section projects" id="projects" aria-labelledby="projects-title"><div className="section-inner"><div className="section-top"><div><SectionLabel number="03">Technical Projects</SectionLabel><h2 className="sr-only" id="projects-title">Technical projects</h2></div><a className="text-link" href="#contact">View all projects <FontAwesomeIcon icon={faArrowRight} /></a></div><div ref={revealRef} className={`carousel-shell reveal-group${visible ? " visible" : ""}`}><button className="carousel-button prev" type="button" aria-label="Previous projects" onClick={() => move(-1)}><FontAwesomeIcon icon={faChevronLeft} /></button><div className="project-viewport" ref={viewportRef} onPointerDown={(event) => { pointerStart.current = event.clientX; }} onPointerUp={finishSwipe} onPointerCancel={() => { pointerStart.current = null; }}><div className="project-track">{projects.map((project) => <article className="project-card" key={project.title}><ProjectVisual type={project.visual} /><h3>{project.title}</h3><p>{project.description}</p><div className="tag-list">{project.tags.map((tag) => <span key={tag}>{tag}</span>)}</div></article>)}</div></div><button className="carousel-button next" type="button" aria-label="Next projects" onClick={() => move(1)}><FontAwesomeIcon icon={faChevronRight} /></button></div><div className="pagination" aria-label="Project pages">{Array.from({ length: maxSlide + 1 }, (_, index) => <button type="button" className={index === activeSlide ? "active" : ""} aria-label={`Show project page ${index + 1}`} aria-current={index === activeSlide ? "true" : undefined} onClick={() => setActiveSlide(index)} key={index} />)}</div></div></section>;
}
...
function Writing() {
  const [revealRef, visible] = useReveal();
  return <section className="section writing" id="writing" aria-labelledby="writing-title"><div className="section-inner"><div className="section-top"><div><SectionLabel number="05">Writing &amp; Insights</SectionLabel><h2 className="sr-only" id="writing-title">Writing and insights</h2></div><a className="text-link" href="#contact">View all writing <FontAwesomeIcon icon={faArrowRight} /></a></div><div ref={revealRef} className={`writing-grid${visible ? " visible" : ""}`}>{writings.map((item) => <article className="writing-card" key={item.title}><div className={`mini-visual ${item.visual}`} aria-hidden="true" /><div><h3>{item.title}</h3><p>{item.summary}</p><a href="#contact">Read <FontAwesomeIcon icon={faArrowRight} /></a></div></article>)}</div></div></section>;
}

function Footer() {
  return <footer className="site-footer" id="contact"><div className="section-inner"><h2>Let&apos;s build what&apos;s next.</h2><p>Open to collaboration on data, systems, and products that create impact.</p><a className="button secondary" href="mailto:garrett.audet@gmail.com">Get in Touch <FontAwesomeIcon icon={faArrowRight} /></a><div className="footer-bottom"><nav aria-label="Social links"><a href="https://www.linkedin.com/in/garrettaudet/" aria-label="LinkedIn"><FontAwesomeIcon icon={faLinkedinIn} /></a><a href="https://github.com/GarrettAudet" aria-label="GitHub"><FontAwesomeIcon icon={faGithub} /></a><a href="https://medium.com/" aria-label="Medium"><FontAwesomeIcon icon={faMedium} /></a><a href="mailto:garrett.audet@gmail.com" aria-label="Email"><FontAwesomeIcon icon={faEnvelope} /></a></nav><p>&copy; 2026 Garrett Audet. All rights reserved.</p></div></div></footer>;
}
