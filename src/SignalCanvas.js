...
const flowVertex = [
  ...
  "  float burst = pow(clamp((x - 0.72) / 0.28, 0.0, 1.0), 0.72);",
  "  float outcomeY = 0.38 + (seed.y - 0.5) * 0.68 * burst + sin(x * 14.0 + seed.x * 6.2 + uTime * 0.2) * (0.018 + seed.z * 0.018);",
...
const lineVertex = [
  ...
  "  float burst = pow(clamp((x - 0.72) / 0.28, 0.0, 1.0), 0.72);",
  "  y = mix(y, 0.38 + (seed.y - 0.5) * 0.68 * burst + sin(x * 14.0 + seed.x * 6.2 + uTime * 0.16) * 0.022, release);",
  "  vec2 p = vec2(x, y);",
  "  vec2 delta = uPointer - p;",
  "  float influence = (1.0 - smoothstep(0.0, 0.24, length(delta))) * uPointerStrength;",
